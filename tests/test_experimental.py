...
def test_experimental_raises_error_for_invalid_version(version: Any) -> None:
    with pytest.raises(ValueError):
        core._experimental.experimental(version)


def test_experimental_decorator() -> None:
    version = "1.1.0"
    decorator_experimental = core._experimental.experimental(version)
    ...
    assert decorated_func.__doc__ == core._experimental._EXPERIMENTAL_NOTE_TEMPLATE.format(
        ver=version
    )
...
def test_experimental_class_decorator() -> None:
    ...
    assert decorated_class.__doc__ == core._experimental._EXPERIMENTAL_NOTE_TEMPLATE.format(
        ver=version
    )
...
def test_experimental_class_decorator_name() -> None:
    ...
    name = "foo"
    decorator_experimental = core._experimental.experimental("1.1.0", name=name)
