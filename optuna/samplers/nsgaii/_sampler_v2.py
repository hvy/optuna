...
from optuna.trial import TrialContext
...
class NSGAIISamplerV2(BaseSamplerV2):
    def __init__(
        self,
        *,
    ...
    def sample_joint(
        ...
        # TODO(hvy): Implement intersection search space with observations
        ...
        parent_generation, parent_population = self._collect_parent_population(ctx)
    ...
    def _collect_parent_population(self, ctx: "optuna.trial.TrialContext") -> Tuple[int, List[FrozenTrial]]:
        generation_to_runnings = defaultdict(list)
        generation_to_population = defaultdict(list)

        observations = ctx.get_observations()

        for obs in observations:
            attrs = ctx.get_trial_attrs(obs.number)

            if _GENERATION_KEY not in attrs:
                continue

            generation = attr.attrs[_GENERATION_KEY]
            if obs.state != optuna.trial.TrialState.COMPLETE:
                if obs.state == optuna.trial.TrialState.RUNNING:
                    generation_to_runnings[generation].append(obs)
                continue

            # Do not use trials whose states are not COMPLETE, or `constraint` will be unavailable.
            generation_to_population[generation].append(obs)

        hasher = hashlib.sha256()
        parent_population: List["optuna.trial.TrialObservation"] = []
        parent_generation = -1
        while True:
            generation = parent_generation + 1
            ...
            for trial in generation_to_runnings[generation]:
                hasher.update(bytes(str(obs.number), "utf-8"))

            cache_key = "{}:{}".format(_POPULATION_CACHE_KEY_PREFIX, hasher.hexdigest())
            study_system_attrs = ctx.study_attrs()
            cached_generation, cached_population_numbers = study_system_attrs.get(
                cache_key, (-1, [])
            )
            if cached_generation >= generation:
                generation = cached_generation
                population = [observations[n] for n in cached_population_numbers]
            else:
                population.extend(parent_population)
                population = self._select_elite_population(ctx.study_directions(), population)
    ...
    def _select_elite_population(
        self, directions: List["optuna.study.StudyDirection"], population: List["optuna.trial.TrialObservation"]
    ) -> List[FrozenTrial]:
        elite_population: List[FrozenTrial] = []
        population_per_rank = self._fast_non_dominated_sort(population, directions)
    ...
    def _fast_non_dominated_sort(
        self,
        population: List["optuna.trial.TrialObservation"],
        directions: List[optuna.study.StudyDirection],
    ) -> List[List[FrozenTrial]]:
        if self._constraints_func is not None:
            for _obs in population:
