...
class TPESamplerV2(BaseSamplerV2):
    ...
    def sample_joint(
        ...
        if self._group:
            ...
            self._search_space_group = self._group_decomposed_search_space.calculate(ctx._study)
        ...
        for name, distribution in self._search_space.calculate(ctx._study).items():
            if distribution.single():
                continue
            search_space[name] = distribution
            ...
            for sub_space in self._search_space_group.search_spaces:
                ...
                for name, distribution in sorted(sub_space.items()):
                    if not distribution.single():
                        search_space[name] = distribution
                params.update(self._sample_relative(ctx, search_space))
            return params
        else:
            return self._sample_relative(ctx, search_space)
    ...
    def _sample_relative(
        self, ctx, search_space: Dict[str, BaseDistribution]
    ) -> Dict[str, Any]:
    ...
    def sample_independent(
        ...
        if any(param is not None for param in values[param_name]):
            # TODO(hvy): Do not require current trial since it is not used.
            self._log_independent_sampling(n, ctx._get_current_trial(), param_name)

        if n < self._n_startup_trials:
            # TODO(hvy): Implement RandomSamplerV2
            return self._random_sampler.sample_independent(
                ctx._study, ctx._get_current_trial(), param_name, param_distribution
            )
        ...
        if len(ctx.get_study_directions()) > 1:
            weights_below = _calculate_weights_below_for_multi_objective(
                scores, indices_below, violations
            )[param_mask_below]
            mpe_below = _ParzenEstimator(
        ...
        log_likelihoods_below = mpe_below.log_pdf(samples_below)
        log_likelihoods_above = mpe_above.log_pdf(samples_below)
        ret = TPESamplerV2._compare(samples_below, log_likelihoods_below, log_likelihoods_above)
...
def _get_observation_pairs(
    ...
    signs = []
    for d in ctx.get_study_directions():
    ...
    for obs in ctx.get_observations():
        if obs.state not in states:
            continue

        # We extract score from the trial.
        if obs.state is TrialState.COMPLETE:
            if obs.values is None:
                continue
            score = (-float("inf"), [sign * v for sign, v in zip(signs, obs.values)])
        elif obs.state is TrialState.PRUNED:
            if len(ctx.study_directions()) > 1:
                continue

            if len(obs.intermediate_values) > 0:
                step, intermediate_value = max(obs.intermediate_values.items())
                ...
                score = (1, [0.0])
        elif obs.state is TrialState.RUNNING:
            if len(ctx.study_direction()) > 1:
                continue
        ...
        for param_name in param_names:
            ...
            if param_name in obs.params:
                distribution = obs.distributions[param_name]
                param_value = distribution.to_internal_repr(obs.params[param_name])
            else:
                param_value = None
        ...
        if constraints_enabled:
            ...
            if obs.state != TrialState.RUNNING:
                constraint = ctx.get_trial_attrs.get(_CONSTRAINTS_KEY)
